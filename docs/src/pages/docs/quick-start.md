...
At the heart of every React Charts is the `useTable` hook and the table `instance` object that it returns. This `instance` object contains everything you'll need to build a table and interact with its state. This includes, but is not limited to:
...
In React Charts, **you the developer** are responsible for rendering the UI (markup and styles) of your table, but don't let that intimidate you! Table UIs are fun and React Charts exists to make the process much easier to wire up your own table UI.
...
> Again, we're using `React.useMemo` so React Charts doesn't recalculate the universe on every single render. Only when the memoized value actually changes!
...
```js
import { useTable } from 'react-charts'
...
Clearly this isn't ready to ship, but from a conceptual standpoint, you just learned the basics of using React Charts!
